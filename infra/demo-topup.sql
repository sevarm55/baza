...
--
-- Открываем в 00:03, а не в девять утра. В девять утра смена, открытая
-- «в будущем», показывала бы ещё не наступившее время. А три минуты, а не
-- десять, потому что первый заезд в 00:06: машина, записанная раньше
-- открытия смены, не попадёт в сдачу наличных при её закрытии.
insert into shifts (tenant_id, user_id, opened_at)
select d.id, u.id, s.at + interval '3 minutes'
