...
async function main() {
  ...
  /* ===================== КОД ПОСЛЕ SMS-ВХОДА ===================== */
  section('У вошедшего по SMS кода нет — но задать его можно');

  {
    const { hasPin, NO_PIN, hashPin, verifyPin } = await import('../lib/pin');

    check('метка «кода нет» распознаётся', !hasPin(NO_PIN));
    check('настоящий хеш — распознаётся', hasPin(await hashPin('481596')));

    /* Главное: к метке не подходит ничего. Иначе владелец, у которого
       кода нет, впускался бы по любому — или по пустой строке. */
    check('к метке не подходит никакой код', !(await verifyPin('481596', NO_PIN)));
    check('и пустой тоже', !(await verifyPin('', NO_PIN)));
    check('и сама метка как код', !(await verifyPin(NO_PIN, NO_PIN)));
  }

  /* ===================== ОКРУЖЕНИЕ ===================== */
  section('Пустая переменная = не задана');

  /* В docker-compose необязательные переменные пишутся как
     `${VAR:-}`, и в контейнер приезжает ПУСТАЯ СТРОКА, а не отсутствие.
     `??` её пропускает как значение — и заголовок авторизации у
     оператора уходил без слова `Bearer`. Оператор отвечал 401 при
     совершенно исправном токене, а в логах было видно только «401». */
  {
    const { env } = await import('../lib/env');
    const key = 'TETR_SECURITY_PROBE';

    process.env[key] = '';
    check('пустая строка читается как «не задано»', env(key) === undefined);

    delete process.env[key];
    check('отсутствующая — тоже', env(key) === undefined);

    process.env[key] = 'Bearer';
    check('заданная возвращается как есть', env(key) === 'Bearer');
    delete process.env[key];
  }
