 ...
 *
 * ЧТО ЭТО ЗА ЧИСЛО. Не «по столько каждому», а вся зарплата машины,
 * которую мыли вдвоём-втроём: `цена × процент` даёт фонд, фонд делится
 * поровну. Пятьдесят процентов на троих — это пять тысяч с
 * десятитысячной машины на всех, а не пятнадцать.
 *
 * Одна ставка на бизнес, а не своя у каждого человека. Свои у каждого
 * означали бы, что у участников ОДНОЙ машины разные условия, и вопрос
 * «под какой процент её помыли» перестал бы иметь один ответ — сначала у
 * владельца, потом и у самих мойщиков между собой.
 *
 * Null выключает свойство: мойщику совместная мойка не предлагается, и
 * сервер её не принимает. Индивидуальный процент при этом работает как
 * работал — моет один, платим по его ставке.
 *
 * Прошлые записи правка не трогает: применённая ставка лежит снимком в
 * каждой записи, а посчитанные доли — в `order_shares`. Поднял ставку в
 * марте — февральские зарплаты остались февральскими.
 */
export async function saveTeamPercent(params: {
  ...
  const [row] = await db
    .update(tenants)
    .set({ teamPercent: params.percent })
