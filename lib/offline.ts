...
export type QueuedOrder = {
   ...
   * запись лежит в очереди, а долю получить должен он сам. Уволенный за
   * это время отвергается сервером, и запись повиснет в очереди с
   * причиной — так же, как повисает запись с удалённой услугой.
   *
   * Необязательное по той же причине, что `serviceIds`: в очереди у
   * людей уже лежат записи без этого поля, и терять их из-за формата
   * нельзя.
   */
