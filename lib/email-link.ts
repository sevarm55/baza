 ...
 * вставляют.
 *
 * Запасное значение зависит от того, где мы работаем, и это не
 * педантичность. В бою переменной может не оказаться — так и было в день
 * перехода на почту, — и общий запасной `localhost` разослал бы живым
 * людям письма со ссылками в никуда. Молча: письмо уходит, ссылка не
 * работает, а понять это можно только по жалобам.
 */
export function origin(): string {
  const set = env('PUBLIC_ORIGIN');
  if (set) return set;
  return process.env.NODE_ENV === 'production' ? 'https://tetrin.pro' : 'http://localhost:3000';
}
