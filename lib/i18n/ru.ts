...
export const ru: Dict = {
  ...
  landing: {
    hero: {
      title: 'Автомойка под вашим контролем',
    ...
    live: {
      title: 'Владелец видит прямо сейчас',
      lead: 'Мойщик записывает машину с телефона, владелец видит выручку, зарплаты и расходы прямо сейчас.',
      demo: 'демо',
      revenue: 'Выручка',
      payroll: 'Зарплата',
    ...
    cabinet: {
      title: 'Весь бизнес на одном экране',
      lead: 'Прибыль, выручка, зарплаты и расходы за любой период. Кто на смене и что записали минуту назад — в той же ленте.',
    },
  ...
  errors: {
    ...
    badPin: 'ПИН должен быть из 6 цифр',
  ...
  auth: {
    ...
    /* ВХОД ПО ПОЧТЕ И ПАРОЛЮ.
       ...
       Ключи заведены рядом со старыми, а не вместо них: пока приложение
       ходит по коду из SMS, оба набора живут вместе. Старые уйдут вместе
       с последним, кто их зовёт. */
    emailLabel: 'Почта',
    ...
    passwordShort: 'Пароль короче 8 знаков',
    passwordCommon: 'Такой пароль подбирают первым. Придумайте другой.',
    passwordMismatch: 'Пароли не совпадают',
    mailFailed: 'Не удалось отправить письмо. Попробуйте позже.',
    readyTitle: 'Готово',
    openApp: 'Открыть приложение',
    backToAppNote: 'Вернитесь в приложение и войдите своим паролем.',
    ...
    /* Подпись страницы, на которую ведёт ссылка из письма. Раньше здесь
       стояла `sentSub` и просила перейти по ссылке человека, который по
       ней уже перешёл. */
    confirmSub: (email: string) => `Адрес ${email} подтверждён. Осталось нажать кнопку.`,

    resetPasswordTitle: 'Восстановление пароля',
    resetPasswordSub: 'Введите почту, пришлём ссылку',
    resetPasswordSend: 'Прислать ссылку',
    newPassword: 'Новый пароль',
    confirmPassword: 'Повторите пароль',
    savePassword: 'Сохранить пароль',
    passwordSaved: 'Пароль изменён',
    passwordSavedNote: 'Все другие устройства вышли. Войдите заново.',

    linkExpired: 'Срок ссылки истёк. Запросите новую.',
    linkInvalid: 'Ссылка не работает. Возможно, ею уже воспользовались.',

    registerEmail: 'Ваша почта',
    registerEmailNote: 'На неё придёт подтверждение и восстановление пароля',
    registerPassword: 'Придумайте пароль',

    changePassword: 'Изменить пароль',
    currentPassword: 'Текущий пароль',
    wrongPassword: 'Текущий пароль неверный',
    passwordChangedNote: 'После изменения выйдут все устройства, включая это.',

    staffPassword: 'Пароль для входа',
    staffPasswordNote: 'Сотрудник входит по своему телефону и этому паролю',
    staffPasswordGenerate: 'Придумать за меня',
    staffPasswordIssued: 'Пароль выдан',
    staffPasswordOnce: 'Запишите или продиктуйте сейчас: второй раз он не покажется.',

    signInTitle: 'Вход',
    note: 'Без карты. За три минуты.',
    phone: 'Номер телефона',
    /* ДВЕ ТАЙНЫ, ДВА СЛОВА, И ОНИ НЕ ПЕРЕСЕКАЮТСЯ.
     *
     * «ПИН» — постоянный, его задаёт человек себе сам или владелец
     * сотруднику. «Код» — одноразовый, приходит сообщением. Слово
     * «код» закреплено за SMS и больше ни за чем.
     *
     * История у этого места такая. Сначала «PIN» стоял разом на обоих,
     * и человек, которому пришла SMS, искал в ней тот код, который ему
     * когда-то продиктовал хозяин мойки. Слово убрали целиком, а
     * постоянный назвали «ПИНом» — и стало хуже: теперь оба
     * назывались словом «код», а спорят они не на экране, а в трубке.
     * Владелец звонит мойщику и говорит «код», и это единственное
     * место, где ошибка стоит смены.
     *
     * Поэтому «ПИН» вернулся, но ровно на одну из двух тайн. Прежняя
     * поломка так не повторяется: одно имя — одна вещь.
     *
     * Внутри системы поле называется `pin` и всегда называлось. */
    pin: 'ПИН',
    signIn: 'Войти',
    signOut: 'Выйти',
    welcomeBack: 'С возвращением',
    tapAvatar: 'Нажмите на аватар, чтобы войти',
    anotherAccount: 'Войти другим аккаунтом',
    rememberedExpired: 'Срок сохранённого входа истёк. Введите логин и пароль.',
    wrongCredentials: 'Неверный номер или пароль',
    ...
    tempAccessTitle: 'Ваш пароль временный',
    tempAccessNote: (deadline: string) => `Он перестанет работать ${deadline}. Задайте свой пароль.`,
    tempAccessAction: 'Задать свой пароль',
    createPin: 'Создайте ПИН',
    ...
    pinTrivial: 'Выберите менее очевидный ПИН',

    changePhone: 'Сменить номер',
    changePhoneNote: 'Номер нужен для связи. Входите вы почтой, поэтому менять его можно свободно.',
    ...
    /* Подсказка сотруднику: откуда у него берутся оба поля сразу.
       Без неё он смотрит на «ПИН» и вспоминает SMS. */
    staffHelper: 'Номер телефона и ПИН выдаёт владелец бизнеса.',
    ...
    ownerCodeHelper: 'ПИН можно создать в профиле Tetrin.',
    accessCodeField: (n: number) => `ПИН · ${n} цифр`,

    /* ───────── ПИН в профиле ─────────
       Три действия вместо прежних двух: создать, изменить, удалить.
       Удаление возвращает человека в то состояние, в котором живёт
       каждый, кто завёл мойку по SMS: ПИНа нет, вход
       только сообщением. */
    deleteAccessCode: 'Удалить ПИН',
    deleteAccessCodeNote: 'Входить останется только по коду из SMS. Выйдут все устройства, включая это.',
    deleteAccessCodeAsk: 'Удалить ПИН?',
    deleteAccessCodeDone: 'ПИН удалён',
    staffAccessCode: 'ПИН сотрудника',
  },
  ...
  firstRun: {
    ...
    s3Note:
      'Работник входит по своему номеру и паролю, открывает смену и записывает вымытые машины.',
    ...
    fCta: 'Начать работу',
    /* Второй выход с финала: номер был выдуман, просто чтобы посмотреть.
       Предложение показывается только для учебной машины. */
    fDropTest: 'Это была проба, убрать',
    fDropTestNote: 'Машина исчезнет из дня и из базы клиентов.',
  },
  ...
  profile: {
    ...
    account: 'Аккаунт',
    phone: 'Телефон',
    /* Адрес входа. Владелец входит именно им, и страница обязана
       показывать то, что он набирает в окне входа. */
    email: 'Почта',
    emailNote: 'Это ваш вход. Чтобы сменить адрес, напишите нам.',
    passwordNote: 'Им вы входите в кабинет и в приложение. После смены выйдут все устройства.',
    access: 'Подписка',
  ...
  payroll: {
    ...
    partial: (sum: string) => `Записана часть выплаты · ${sum}. Проверьте список`,
    nothingOwed: 'Эти дни уже отмечены выплаченными',
  ...
  today: {
    ...
    /* Та же панель, когда на смене никого: люди работали сегодня, но
       сейчас их нет. Слово «сейчас» при закрытой смене спорило с
       соседней строкой «0 мойщиков на смене». */
  ...
  settings: {
    ...
    staffNote:
      'Сотрудник входит со своего телефона и этим паролем. Продиктуйте пароль ему: себе запоминать не нужно.',
    removeStaffNote:
    ...
    passwordIssue: 'Выдать новый пароль',

    passwordIssueNote: 'Сотрудник забыл пароль. Назначьте новый, входы старым закроются.',


    passwordWorksElsewhere: 'Этот человек работает не только здесь. Пароль он меняет сам, из своего профиля.',
