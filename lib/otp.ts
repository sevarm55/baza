...
import { DEFAULT_AUTH_LOCALE, isAuthLocale, type AuthLocale } from './i18n/auth';
 ...
 */
const SMS_TEXT: Record<AuthLocale, (code: string) => string> = {
  hy: (code) => `Tetrin: ${code}\nՄի փոխանցեք ոչ ոքի`,
  ru: (code) => `Tetrin: ${code}\nНикому не сообщайте`,
  en: (code) => `Tetrin: ${code}\nDo not share this code`,
};

function smsText(code: string, locale: AuthLocale): string {
  const text = (SMS_TEXT[locale] ?? SMS_TEXT[DEFAULT_AUTH_LOCALE])(code);
 ...
 */
export function smsPreview(code: string, locale: AuthLocale): string {
...
export async function startChallenge(input: {
  ...
  const code = makeCode();
  const locale: AuthLocale = isAuthLocale(input.locale) ? input.locale : DEFAULT_AUTH_LOCALE;
  const now = Date.now();
...
export async function resendChallenge(input: {
  ...
  const locale: AuthLocale = isAuthLocale(stored?.__locale) ? stored.__locale : DEFAULT_AUTH_LOCALE;
