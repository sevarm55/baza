...
enum API {
    ...
    /**
     * Сколько знаков в пароле, самое малое.
     *
     * То же число, что на сервере (`PASSWORD_MIN` в `lib/password.ts`),
     * и по нему же сервер отказывает.
     *
     * Здесь оно одно на всё приложение, и это не аккуратность. До этого
     * в трёх формах руками стояла длина ПИНа — найм, смена кода,
     * удаление бизнеса, — и все три молча отправляли на сервер заведомо
     * негодное: снаружи это выглядело как «сервер отказал», а не как
     * «слишком коротко».
     */
    ...
    struct Tenant: Decodable {
        ...
        let name: String
        let currency: String
        /// Валюту уже не сменить: в мойке есть записи. Первая же машина
        /// закрывает выбор навсегда — суммы лежат в валюте, и пересчитать
        /// их не по чему.
...
actor APIClient {
    ...
    #else
    /**
     * Та же метка в магазинной сборке: пустая, и другой быть не может.
     *
     * Контур там ровно один — бой, — а имена ключей обязаны остаться
     * прежними: смена хотя бы одного вышибла бы из аккаунта всех живых
     * клиентов разом, прямо на обновлении.
     *
     * Ветка не косметика. `Session` складывает эту метку в имена ключей
     * безусловно, и без неё Release не собирается вовсе: «type
     * 'APIClient' has no member 'scope'», пять раз. Debug при этом
     * собирался, поэтому не видно было до самой архивации.
     */
    static var scope: String { "" }
    #endif
