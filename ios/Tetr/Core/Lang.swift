...
final class LangStore: ObservableObject {
     ...
     * Первый запуск.
     *
     * Порядок короткий: выбор человека → армянский. Спрашивать на первом
     * экране нечего: у мойщика на площадке стоит клиент, и вопрос про
     * язык там не к месту.
     *
     * Язык телефона больше не смотрим, хотя раньше смотрели. Продукт
     * работает в Армении, и его язык армянский; телефон при этом у
     * половины людей русский или английский с прошлой жизни, и они
     * получали интерфейс, который владелец не заказывал. Кому нужен
     * другой язык, переключает его в один тап на экране входа, и выбор
     * запоминается.
     */
