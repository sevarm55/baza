...
final class OrderQueue: ObservableObject {
    struct Item: Codable, Identifiable {
         ...
         *
         * Идентификаторами, а не именами: имя человек может сменить, пока
         * запись лежит в телефоне, а долю получить должен он сам.
         * Уволенного за это время сервер отвергнет, и запись повиснет в
         * очереди с причиной — ровно как запись с удалённой услугой.
         *
         * Необязательное: в очереди лежат записи, сделанные до совместной
         * работы, и терять их из-за формата нельзя.
         */
    ...
    func flush(using session: Session) async -> Int {
        ...
        for item in items where item.failure == nil && (item.tenantId == nil || item.tenantId == here) {
            do {
                ...
                /* Состав едет вместе с записью. Проверяет его сервер:
                   уволенный за время лежания в очереди отвергается там, а
                   не здесь. */
                if let crew = item.participants, !crew.isEmpty {
                    payload["participants"] = crew
                }
