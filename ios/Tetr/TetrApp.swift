        ...
        NavigationStack {
            ShiftView()
                ...
                .toolbar {
                    ...
                    /* Язык на смене — только мойщику.
                     *
                     * Раньше смена была первой вкладкой у обоих, и глобус
                     * стоял на виду у каждого. Теперь у владельца первой
                     * идёт сводка, а смена уехала в отдельный выступ
                     * справа, и язык там оказался спрятан именно от того,
                     * кому есть чем его заменить: у владельца он лежит
                     * строкой в «Профиль и вход».
                     *
                     * У мойщика профиля нет вовсе, и смена — единственный
                     * его экран. Для него глобус остаётся здесь.
                     */
    ...
    private var languageMenu: some ToolbarContent {
        ToolbarItem(placement: .topBarLeading) {
            Menu {
                ...
                /* Цвет не задаём: знак наследует схему панели, а её
                   переключает сама смена по прокрутке. Прибитый белый
                   пережил бы это переключение и остался бы белым на
                   белом листе — ровно так кнопка выхода и пропадала. */
    ...
    private var signOut: some ToolbarContent {
        ToolbarItem(placement: .topBarTrailing) {
            Button {
                ...
                /* Как и глобус: цвет от схемы панели, а не прибитый.
                   На сцене схема тёмная и знак белый, на белом листе
                   светлая и знак тёмный. */
