...
struct ExpiredView: View {
     ...
     * Хозяин ли смотрит.
     *
     * Мойщику здесь показывали то же, что владельцу: «ваши данные, ваша
     * выручка, ваша база клиентов» и кнопку «удалить бизнес». Сервер её,
     * конечно, не пускал — но кнопка была, и текст обращался не к тому
     * человеку. Мойщику нужен один ответ: закрыто не у тебя, спроси
     * хозяина.
     */
    private var isOwner: Bool { session.me?.isOwner == true }

    /* Владельцу — про его данные и что с ними ничего не случилось.
       Работнику — коротко и про него: записывать сейчас нельзя, решает
       не он. Обещать ему «ваши данные целы» бессмысленно: данные не
       его. */
    private var ownerText: String {
        guard isOwner else {
            return "Այս բիզնեսում գրանցումները ժամանակավորապես փակ են։ Դիմեք սեփականատիրոջը։"
        }
        return fresh
            ? "Սկսում ենք վճարումից հետո։ Ձեր մյուս բիզնեսներն աշխատում են ինչպես առաջ։"
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            ...
            VStack(alignment: .leading, spacing: 14) {
                Text(isOwner ? (fresh ? "Բիզնեսը ստեղծված է" : "Ժամկետը լրացել է") : "Մուտքը փակ է")
