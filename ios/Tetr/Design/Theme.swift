...
enum Brand {
    ...
    static func sign(_ amount: Int) -> Color {
        /* Краску получает только беда. Положительное число — чернилами:
           гигантская зелёная цифра читалась виджетом-дашбордом, а не
           документом, и владелец попросил профессиональнее. Зелёный
           остаётся у дельт, состояний смены и «получилось». */
        amount < 0 ? badOnBoard : onBoard
    }
