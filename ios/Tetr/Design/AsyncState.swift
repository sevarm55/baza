...
/**
 * Марка вместо пустоты, пока едут данные.
 *
 * Скелет отвечает на вопрос «какой формы будет экран», и это правильный
 * ответ там, где форма известна заранее. Но он молчит о том, что вообще
 * что-то происходит: очень мягкая волна по серым прямоугольникам
 * читается как «экран уже такой», а не как «сейчас будет». Полторы
 * секунды такого молчания владелец описывает словами «ничего не видно,
 * потом появилось».
 *
 * Здесь работает знак продукта: четыре столбика, которые складываются в
 * квадрат и расходятся обратно. Он говорит ровно одно, зато внятно — идёт
 * загрузка, — и говорит это маркой, а не серым.
 *
 * Высота задаётся снаружи: загрузчик обязан занять то же место, что
 * займёт содержимое, иначе экран прыгнет в момент подстановки.
 */
struct TetrScreenLoader: View {
    /// Сколько места держать под будущим содержимым.
