...
import { hasMoney } from '@/lib/profile';
  ...
  try {
    ...
    return ok({
      tenant: {
        ...
        /* Можно ли ещё её сменить. Первые же записанные деньги — машина,
           расход, выплата или абонемент — закрывают выбор навсегда:
           суммы лежат в валюте, и пересчитать их не по чему. */
        currencyLocked: await hasMoney(ctx.tenant.id),
      ...
      crew: {
        ...
        /* Владельца здесь нет. Он не моет: отметить его участником
           значит назначить долю тому, кто её не берёт. Ровно так же
           устроен список на вебе и список мойщиков на экране команды. */
        members: staff
      ...
      me: {
        ...
        /* Чем человек входит, тем ему и показываемся: у владельца это
           почта, у сотрудника телефон. Раньше здесь стоял один телефон с
           подписью «это логин», и с переходом на пароль владелец видел в
           профиле не ту строку, которую набирает в окне входа. */
