 ...
 *
 * На широком экране стоит слева и едет вместе с прокруткой, на узком
 * превращается в строку ссылок над панелями. Подсветки текущего раздела
 * нет намеренно: страница прокручивается целиком, и «где я» видно по
 * заголовку панели.
 */
export function SubNav({ label, items }: { label: string; items: { id: string; label: ReactNode }[] }) {
  return (
    <nav aria-label={label} className="lg:w-48 lg:shrink-0">
