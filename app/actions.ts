...
export async function addOrder(input: {
  ...
  await createOrder({
    ...
    /* Режим «глазами работника» из первого запуска. Запись обычная во
       всём, кроме одного: на финале обучения её предложат убрать, если
       номер был выдуман ради посмотреть. */
    fromPreview: session.preview === true,
  });
